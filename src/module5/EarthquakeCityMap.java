 ...
 * team
 * 
 * @author Vandit Goel Date: September 15, 2018
 */
public class EarthquakeCityMap extends PApplet {
	...
	// methods)
	// we will use many of these variables, but the only one we should need to add
	// code to modify is countryQuakes, where we will store the number of
	// earthquakes
	// per country.

	// we can ignore this. It's to get rid of eclipse warnings
		...
		for (Marker m : quakeMarkers) {
			if (!m.isHidden() && m.isInside(map, mouseX, mouseY)) {
				...
				for (Marker mhide : cityMarkers) {
					...
					if (eqm.getDistanceTo(mhide.getLocation()) > eqm.threatCircle()) {
						mhide.setHidden(true);
					}
				}
			}
